...
async def lifespan(application: FastAPI):
    ...
    print(
        f"  Network: http://<this-machine-IP>:{config.PORT}  (use for other lab machines)"
    )
    print(
        f'  Admins:  {", ".join(config.ADMIN_USERNAMES)} (set ADMIN_USERNAMES env var to change)\n'
    )
    ...
    # Close the pooled async HTTP client cleanly.
    await store.aclose()
...
async def post_message(request: Request):
    ...
    if not msg_text:
        return JSONResponse(
            {"error": "Message content cannot be empty"}, status_code=400
        )
    ...
    room = (
        data.get("room") or request.query_params.get("room") or config.DEFAULT_ROOMS[0]
    )
    ...
    # Encrypt (AES-GCM), Sign (Ed25519), and Store via the async DB-server client
    saved = await store.append_message(room, msg_obj)
...
async def get_feed_route(room: str = None, limit: int = 50):
    ...
    """
    return await store.get_feed(room_id=room, limit=limit)


@app.get("/health")
async def health():
    """
    Cheap liveness check for the load balancer's health checker.
    Deliberately does NOT touch the DB server or decrypt/verify anything —
    /feed is expensive (decrypts + signature-verifies up to `limit`
    messages) and shouldn't be hit every couple of seconds by the LB.
    """
    return {"status": "ok"}
